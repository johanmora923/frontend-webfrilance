...
export const OfferCard = ({ Icon, title, startingPrice, maintenanceFee, description, buttonText, buttonId, buttonData }) => {
    ...
    return (
        <div className="ofert__container bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
            ...
            <p className="p__ofert text-gray-600 text-center mb-6">{description}</p>
            <button
                id={buttonId}
                data-servic={buttonData}
                onClick={ToContact}
                className="btn__contact bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium p-3 rounded-lg w-full hover:from-blue-600 hover:to-indigo-700 transition-all duration-300"
            >
                {buttonText}
            </button>
        </div>
